...
function AttributeTable({ rows }: { rows: Record<string, unknown>[] }) {
...
export default function MapAnalysisClient({ user, analysis }: Props) {
  ...
  const nicadToProps = useMemo<Map<string, Record<string, unknown>>>(() => {
    ...
    if (!analysis.geoJsonData) return m;
    try {
      const fc = JSON.parse(analysis.geoJsonData) as {
  ...
  const duplicateGroups = useMemo(() => {
    ...
    for (const e of analysis.errors) {
      if (e.errorType !== "duplicate" || !e.nicad1) continue;
  ...
  const selectDuplicateGroup = useCallback(
    (group: { nicad: string; errors: GeoError[]; objectIds: string[] }) => {
      ...
      setSelectedError(group.errors[0] ?? null); // fit map to first occurrence
      const allFeats = nicadToAllFeatures.get(group.nicad) ?? [];
      if (allFeats.length > 0) {
        setTableRows(
          allFeats.map((p, i) => ({
            _role: `Occurrence ${i + 1}`,
            ...p,
          }))
        );
        setTableOpen(true);
  ...
  }, [nicadSearch, nicadToAllFeatures]);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 relative min-h-0">
            <MapLibreMap
              geoJson={correctedData ?? analysis.geoJsonData}
            ...
            <button
              ...
              className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1.5 px-3 py-1.5 text-[11px] rounded-full bg-card/90 backdrop-blur-sm border border-border shadow-lg hover:bg-card transition-colors"
            >
          ...
          {tableOpen && (
            <div className="border-t border-border bg-card shrink-0 flex flex-col" style={{ height: 210 }}>
              ...
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-border shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  {tableRows.length > 0 && (
                    <span className="text-[10px] text-muted-foreground flex items-center gap-2">
                      ...
                      <button
                        onClick={() => { setTableRows([]); setSelectedParcel(null); }}
                ...
                <button
                  onClick={() => { setTableOpen(false); setTableRows([]); }}
