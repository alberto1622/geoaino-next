...
const STAGES: Array<{ key: StageKey; title: string; desc: string; out: string }> = [
  { key: "lecture", title: "Lecture", desc: "Chaque entité du DXF est décodée calque par calque.", out: "1 284 entités" },
  { key: "mappage", title: "Mappage des calques", desc: "Les calques sont associés à leur classe cadastrale.", out: "4 calques → 3 classes" },
  { key: "polygonisation", title: "Polygonisation", desc: "Les segments épars se referment en parcelle valide.", out: "1 anneau, 5 sommets" },
  { key: "jointure", title: "Jointure spatiale", desc: "La parcelle rejoint sa commune : le NICAD s’assemble.", out: "Syscol attribué" },
];
...
const LECTURE_SEGS = (() => {
  ...
  const out: Array<{ x1: number; y1: number; x2: number; y2: number; y: number }> = [];
  for (let i = 0; i < 12; i++) {
    const x = 0.1 + rnd() * 0.8, y = 0.08 + rnd() * 0.84;
    const len = 0.06 + rnd() * 0.12, ang = rnd() * Math.PI * 2;
    out.push({ x1: x, y1: y, x2: x + Math.cos(ang) * len, y2: y + Math.sin(ang) * len, y });
  }
  return out;
})();
function drawLecture(ctx: CanvasRenderingContext2D, w: number, h: number, p: number, c: Colors) {
  const scanY = h * ease(Math.min(p / 0.82, 1));
  for (const s of LECTURE_SEGS) {
    if (s.y * h <= scanY + 4) {
      ctx.strokeStyle = c.ink; ctx.lineWidth = 1.2; ctx.globalAlpha = 0.85;
      ctx.beginPath(); ctx.moveTo(s.x1 * w, s.y1 * h); ctx.lineTo(s.x2 * w, s.y2 * h); ctx.stroke();
      ctx.globalAlpha = 1;
    }
  }
  ctx.strokeStyle = c.accent; ctx.lineWidth = 1.5; ctx.globalAlpha = 0.9;
  ctx.beginPath(); ctx.moveTo(0, scanY); ctx.lineTo(w, scanY); ctx.stroke();
  ctx.globalAlpha = 0.12; ctx.fillStyle = c.accent; ctx.fillRect(0, Math.max(0, scanY - 10), w, 10);
  ctx.globalAlpha = 1;
}
...
const MATCH = [0, 1, 2];
function roundRect(ctx: CanvasRenderingContext2D, x: number, y: number, wr: number, hr: number, r: number) {
  ctx.beginPath();
  ctx.moveTo(x + r, y); ctx.arcTo(x + wr, y, x + wr, y + hr, r); ctx.arcTo(x + wr, y + hr, x, y + hr, r);
  ctx.arcTo(x, y + hr, x, y, r); ctx.arcTo(x, y, x + wr, y, r); ctx.closePath();
}
function drawMappage(ctx: CanvasRenderingContext2D, w: number, h: number, p: number, c: Colors) {
  const leftX = w * 0.06, rightX = w * 0.62;
  const topPad = h * 0.14, rowH = (h - topPad * 1.6) / LAYERS.length;
  ...
  function chip(x: number, y: number, text: string, hi: boolean): number {
    ...
    ctx.fillStyle = hi ? c.accentSoft : c.card; ctx.strokeStyle = hi ? c.accent : c.border; ctx.lineWidth = 1;
    roundRect(ctx, x, y - 9, wtxt, 18, 3); ctx.fill(); ctx.stroke();
    ctx.fillStyle = hi ? c.accent : c.ink; ctx.fillText(text, x + 7, y + 1);
    return wtxt;
  }

  const leftY = LAYERS.map((_, i) => topPad + rowH * i + rowH / 2);
  const rh = h - topPad * 1.6;
  const rightY = CLASSES.map((_, i) => topPad + (rh / CLASSES.length) * i + rh / (CLASSES.length * 2));

  const per = 1 / LAYERS.length;
  LAYERS.forEach((name, i) => {
    const localP = Math.min(Math.max((p - i * per * 0.7) / (per * 1.4), 0), 1);
    const drawn = ease(localP);
    const y0 = leftY[i], y1 = rightY[MATCH[i]];
    ctx.globalAlpha = 0.2 + drawn * 0.8;
    const cw = chip(leftX, y0, name, false);
    ctx.globalAlpha = 1;
    if (drawn > 0.04) {
      const sx = leftX + cw + 2, ex = rightX - 10;
      const endX = sx + (ex - sx) * drawn, endY = y0 + (y1 - y0) * drawn;
      ctx.strokeStyle = c.inkDim; ctx.lineWidth = 1; ctx.globalAlpha = 0.75;
      ctx.beginPath(); ctx.moveTo(sx, y0); ctx.bezierCurveTo(sx + 30, y0, ex - 24, y1, endX, endY); ctx.stroke();
      ctx.globalAlpha = 1;
      if (drawn < 1) { ctx.fillStyle = c.accent; ctx.beginPath(); ctx.arc(endX, endY, 2.2, 0, Math.PI * 2); ctx.fill(); }
    }
  });
  CLASSES.forEach((name, i) => {
    const hi = MATCH.some((m, li) => {
      const localP = Math.min(Math.max((p - li * per * 0.7) / (per * 1.4), 0), 1);
...
/* ── Polygonisation ─────────────────────────────────────────────────── */
const PARCEL_PTS = [[0.32, 0.2], [0.7, 0.16], [0.82, 0.56], [0.54, 0.84], [0.2, 0.6]].map(([x, y]) => ({ x, y }));
const SEG_START = (() => {
  const rnd = mulberry32(42);
  return PARCEL_PTS.map(() => ({ dx: (rnd() - 0.5) * 0.9, dy: (rnd() - 0.5) * 0.9 }));
})();
function drawPolygonisation(ctx: CanvasRenderingContext2D, w: number, h: number, p: number, c: Colors) {
  const n = PARCEL_PTS.length, per = 1 / n;
  const pts = PARCEL_PTS.map((t, i) => {
    const localP = ease(Math.min(Math.max((p - i * per * 0.55) / (per * 2.2), 0), 1));
    const s = SEG_START[i];
    return { x: lerp(t.x + s.dx, t.x, localP) * w, y: lerp(t.y + s.dy, t.y, localP) * h, done: localP > 0.985 };
  });
  ...
  if (allDone) {
    ...
    ctx.closePath();
    ctx.fillStyle = c.parcelSoft; ctx.fill();
    ctx.strokeStyle = c.parcel; ctx.lineWidth = 2; ctx.stroke();
  } else {
    for (let j = 0; j < n; j++) {
      const a = pts[j], b = pts[(j + 1) % n];
      ctx.strokeStyle = c.ink; ctx.lineWidth = 1.3; ctx.globalAlpha = 0.85;
  ...
  pts.forEach((pt) => {
    ...
    ctx.beginPath(); ctx.arc(pt.x, pt.y, pt.done ? 2.6 : 1.8, 0, Math.PI * 2); ctx.fill();
    if (pt.done) {
      const pulse = (performance.now() / 260) % 1;
      ctx.strokeStyle = c.accent; ctx.globalAlpha = 1 - pulse; ctx.lineWidth = 1.2;
      ctx.beginPath(); ctx.arc(pt.x, pt.y, 3 + pulse * 8, 0, Math.PI * 2); ctx.stroke();
      ctx.globalAlpha = 1;
    }
  });
}
...
/* ── Jointure spatiale ─────────────────────────────────────────────── */
const COMMUNE_PTS = [[0.06, 0.3], [0.3, 0.06], [0.72, 0.1], [0.94, 0.42], [0.86, 0.82], [0.46, 0.94], [0.1, 0.7]].map(([x, y]) => ({ x, y }));
const NICAD_STR = "0143 0121 001 00042";
function drawJointure(ctx: CanvasRenderingContext2D, w: number, h: number, p: number, c: Colors) {
  ctx.setLineDash([3, 3]); ctx.strokeStyle = c.inkFaint; ctx.lineWidth = 1.1;
  ctx.beginPath();
  COMMUNE_PTS.forEach((pt, i) => { const x = pt.x * w, y = pt.y * h; i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y); });
  ctx.closePath(); ctx.stroke(); ctx.setLineDash([]);

  const cx = 0.5 * w, cy = 0.46 * h, scale = 0.32;
  ctx.beginPath();
  PARCEL_PTS.forEach((pt, i) => {
    const x = cx + (pt.x - 0.5) * w * scale, y = cy + (pt.y - 0.5) * h * scale;
    i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
  });
  ctx.closePath();
  ctx.fillStyle = c.parcelSoft; ctx.fill();
  ctx.strokeStyle = c.parcel; ctx.lineWidth = 1.5; ctx.stroke();
  ...
  if (pingP < 1) {
    [0, 0.4].forEach((delay) => {
      ...
      if (lp <= 0 || lp >= 1) return;
      ctx.strokeStyle = c.accent; ctx.globalAlpha = 1 - lp; ctx.lineWidth = 1.3;
      ctx.beginPath(); ctx.arc(cx, cy, lp * w * 0.4, 0, Math.PI * 2); ctx.stroke();
      ctx.globalAlpha = 1;
    });
  }
  if (p > 0.5) {
    const bp = ease(Math.min((p - 0.5) / 0.2, 1));
    ctx.setLineDash([3, 3]); ctx.strokeStyle = c.accent; ctx.globalAlpha = bp; ctx.lineWidth = 1.6;
    ctx.beginPath();
    COMMUNE_PTS.forEach((pt, i) => { const x = pt.x * w, y = pt.y * h; i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y); });
    ctx.closePath(); ctx.stroke(); ctx.setLineDash([]); ctx.globalAlpha = 1;
  }
  if (p > 0.6) {
    const chars = Math.floor(ease(Math.min((p - 0.6) / 0.38, 1)) * NICAD_STR.length);
    ctx.font = "600 " + Math.round(w * 0.058) + `px ${c.monoFont}, ui-monospace, monospace`;
...
const DRAW: Record<StageKey, (ctx: CanvasRenderingContext2D, w: number, h: number, p: number, c: Colors) => void> = {
  lecture: drawLecture,
  mappage: drawMappage,
...
const CHEVRON = (
  <svg viewBox="0 0 18 10" fill="none" aria-hidden="true" className="h-[10px] w-[18px]">
    <path d="M1 1l8 4-8 4M9 1l8 4-8 4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
...
export default function DxfPipelineSection() {
  ...
  useEffect(() => {
    ...
    const sizes: Array<{ w: number; h: number }> = STAGES.map(() => ({ w: 200, h: 200 }));
    ...
    const monoFont = getComputedStyle(document.documentElement).getPropertyValue("--font-geist-mono").trim() || "ui-monospace";
    ...
    function frame(now: number) {
      ...
      const currentIndex = Math.min(Math.floor(cyclePos / STAGE_WINDOW_MS), STAGES.length);
      ...
      STAGES.forEach((s, i) => {
        ...
        if (frameEl) frameEl.style.borderColor = state === "active" ? colors.accent : colors.border;
        if (numEl) numEl.style.color = state === "active" ? colors.accent : state === "done" ? colors.inkDim : colors.inkFaint;
    ...
    <section className="max-w-7xl mx-auto px-6 py-16">
      ...
      <div
        ...
        style={{
          ...
          "--dxf-ink-faint": "color-mix(in oklab, var(--muted-foreground) 60%, transparent)",
          "--dxf-accent": "var(--ring)",
            ...
            <div key={s.key} className="contents">
              ...
              >
                <div className="flex items-baseline gap-2 mb-2.5">
                  <span ref={(el) => { numElsRef.current[i] = el; }} className="font-mono text-[11px] font-bold transition-colors duration-300">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="text-[13.5px] font-bold">{s.title}</span>
                </div>
                <div
                  ref={(el) => { frameElsRef.current[i] = el; }}
                ...
                >
                  <canvas ref={(el) => { canvasElsRef.current[i] = el; }} className="absolute inset-0 h-full w-full" />
                </div>
                <div className="mt-2.5 h-[2px] w-full rounded-full bg-border relative overflow-hidden">
                  <div ref={(el) => { fillElsRef.current[i] = el; }} className="absolute inset-y-0 left-0 w-0" style={{ background: "var(--ring)" }} />
                </div>
                ...
                <p className="mt-auto pt-2.5 font-mono text-[11px] text-muted-foreground">
                  Sortie <span className="font-semibold text-foreground">{s.out}</span>
                </p>
              </div>
              {i < STAGES.length - 1 && (
                <div
                  ref={(el) => { connectorElsRef.current[i] = el; }}
                  aria-hidden="true"
